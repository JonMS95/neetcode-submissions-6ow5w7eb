...
public:
    mat_int kClosest(mat_int& points, int k)
    {
        ...
        using pq_min_dist = std::priority_queue<pair_pt_dist, vec_pt_dist, decltype(heap_crit)>;

        pq_min_dist min_dist_heap(points_to_distances.begin(), points_to_distances.end(), heap_crit);
