...
private:
	...
	std::vector<std::string> _getAdjacentWords(const std::string& cur, const std::vector<std::string>& word_list)
	{
	...
	int _bfs(const std::string& start, const std::vector<std::string>& word_list, const std::string& end)
	{
		...
		while(!cur.empty())
		{
			...
			for(int i = 0; i < queue_size; i++)
			{
				...
				for(const std::string& adj : this->_getAdjacentWords(cur.front(), word_list))
					if(!known.count(adj))
						cur.push(adj);
